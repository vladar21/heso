...
def schedule(request):
    ...
    for lesson in lessons:
        ...
        student_usernames_ids = list(
            lesson.english_class.students.values("id", "username")
        )
    ...
    return render(
        ...
        {"lessons_list": json.dumps(lessons_data), "is_readonly": is_readonly},
    )
...
def lesson_details(request):
    ...
    if not request.user.is_authenticated:
        return JsonResponse(
            {"status": "error", "message": "Unauthorized access. Please log in."},
            status=403,
        )
    ...
    if not (
        ...
        or request.user == lesson.english_class.teacher
        or request.user in lesson.english_class.students.all()
    ):
        return JsonResponse(
            {
                "status": "error",
    ...
    lesson_data = {
        ...
        "extendedProps": {
            ...
            "location": lesson.location,
            "teacher_id": (
                lesson.english_class.teacher.id
                if lesson.english_class.teacher
                else None
            ),
            "teachers": [
                {"id": teacher.id, "username": teacher.username} for teacher in teachers
            ],
            "students": [
                {"id": student.id, "username": student.username} for student in students
            ],
            "materials": [
                {"id": material.id, "title": material.title} for material in materials
            ],
        },
    }
...
def update_lesson(request):
    ...
    try:
        ...
        if not (
            request.user.is_superuser or request.user == lesson.english_class.teacher
        ):
            return JsonResponse(
                {
                    "status": "error",
        ...
        with transaction.atomic():
            ...
            if "start" in data:
                lesson.start_time = parse_datetime(data["start"]).astimezone(
                    timezone.get_default_timezone()
                )
            if "end" in data:
                lesson.end_time = parse_datetime(data["end"]).astimezone(
                    timezone.get_default_timezone()
                )
            if "location" in data:
            ...
            if "meeting_link" in data and lesson.location == "online":
                ...
                lesson.meeting_link = (
                    None  # Clear meeting link if location is not online
                )
            ...
            if "teacher" in data:
                ...
                try:
                    teacher = User.objects.filter(
                        pk=teacher_id, is_teacher=True
                    ).first()
                    lesson.english_class.teacher = teacher
                    lesson.english_class.save()
                    total_lessons = lesson.english_class.lessons.count()
                    lesson_number = (
                        list(lesson.english_class.lessons.order_by("start_time")).index(
                            lesson
                        )
                        + 1
                    )
            ...
            lesson_number = (
                list(updated_lesson.english_class.lessons.order_by("start_time")).index(
                    updated_lesson
                )
                + 1
            )
            ...
            students = User.objects.filter(
                is_student=True, enrolled_classes__lessons=updated_lesson
            )
        ...
        return JsonResponse(
            {"status": "error", "message": "One or more related objects not found."},
            status=404,
        )
    except Exception as e:
        # General exception catch to handle unforeseen errors
        return JsonResponse(
            {"status": "error", "message": f"Unexpected error: {str(e)}"}, status=400
        )
...
def update_english_class(request, pk):
    ...
    if not (
        request.user.is_superuser
        or is_teacher
        or request.user in english_class.students.all()
    ):
    ...
    if request.method == "POST":
        ...
        class_form = EnglishClassForm(
            request.POST or None, instance=english_class, user=request.user
        )
        ...
        if request.user.is_superuser:
            ...
            if request.user.is_teacher:
                ...
                schedules = Schedule.objects.filter(
                    english_class__students=request.user
                )
        return render(
            request, "scheduling/english_class_list.html", {"schedules": schedules}
        )
    else:
...
def delete_english_class(request, pk):
    ...
    if not (
        request.user.is_superuser or request.user == schedule.english_class.teacher
    ):
    ...
    return render(
        request,
        "scheduling/delete_english_class.html",
...
def create_lesson(request, class_id):
    ...
    if not (
        request.user.is_superuser
        or (request.user.is_teacher and request.user == english_class.teacher)
    ):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for file in new_materials_files:
                if not Material.objects.filter(title=file.name).exists():
                    ...
                    messages.error(
                        request,
                        f'A material with the name "{file.name}" already exists.',
                    )
                lesson.save()
        ...
        form.fields["students"].initial = [
            student.id for student in english_class.students.all()
        ]

    return render(
        request, "scheduling/lesson_form.html", {"form": form, "class_id": class_id}
    )
...
def lessons_list(request, class_id):
    ...
    lessons = Lesson.objects.filter(english_class=english_class).prefetch_related(
        "english_class__teacher", "english_class__students"
    )
    return render(
        request,
        "scheduling/lessons_list.html",
        {
            "lessons": lessons,
            "english_class": english_class,
            "class_id": class_id,
...
def update_lesson_view(request, pk):
    ...
    is_student = not request.user.is_superuser and not is_teacher
    class_id = lesson.english_class.id
    ...
    if not (
        ...
        return redirect("lessons_list", class_id)
    else:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if "students" in request.POST:
                    updated_lesson.english_class.students.set(
                        request.POST.getlist("students")
                    )
                if "materials" in request.POST:
                    material_ids = request.POST.getlist("materials")
                    updated_lesson.materials.set(
                        Material.objects.filter(id__in=material_ids)
                    )
    ...
    return render(
        request,
        "scheduling/lesson_form.html",
        {"form": form, "lesson": lesson, "is_student": is_student},
    )
